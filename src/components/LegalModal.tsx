        ...
        <div className="flex-1 overflow-y-auto p-6 text-slate-700 text-xs space-y-6 leading-relaxed">
          {activeTab === 'disclaimer' && (
            <div className="space-y-4">
              ...
              <p>
                <strong>1.1 No Professional Advice:</strong> SimplyTools provides digital calculation, estimation, and documentation software tools including: Goods and Services Tax (GST) computation, GST Invoice & Payment Link Generation, Cost-Plus pricing models, Income Tax (Old vs. New regimes) projections, TDS (Tax Deducted at Source) splits, House Rent Allowance (HRA) exemptions, Startup Cash Runway / Burn rates, and Working Capital cycles. This application is designed solely for preliminary, self-educational reference and draft planning. It does not contain, constitute, represent, or substitute for professional Chartered Accountant (CA) services, registered financial advisory, audit consultations, or legal planning.
              </p>
              <p>
              ...
              <p>
                <strong>1.3 GST Invoice & Sample Payment Links Disclaimer:</strong> The GST Invoice Generator and Payment Link Customizer are provided strictly for draft invoicing layouts and simulated visualization. Any payment URLs generated or formatted (such as custom prefix <code className="bg-slate-100 px-1 py-0.5 rounded font-mono text-[10px]">rzp.io/l/pay_*</code> links) are non-functional samples for draft preview purposes. To collect actual settlements from your clients, you must create an official merchant account with Razorpay, Stripe, or other RBI-regulated payment gateways and obtain functional, secure links. SimplyTools is not a payment gateway, does not process actual transactions, and does not hold or handle commercial funds.
              </p>
              <p>
                <strong>1.4 Compliance with CGST Act 2017:</strong> Generating an invoice using our PDF download feature does not constitute official e-Invoicing filing with the Invoice Registration Portal (IRP) or GSTN of India. Users are solely responsible for ensuring that all issued invoices conform strictly to Rule 46 of the CGST Rules, 2017, including sequential invoice numbering, correct Place of Supply determination (IGST vs. CGST+SGST split based on supplier and billing states), and timely e-way bill generation or GST returns filing.
              </p>
              <p className="bg-slate-50 p-3 rounded-lg border border-slate-100 font-medium text-amber-900 bg-amber-50/40 border-amber-200/50">
                <strong>1.5 ASSUMPTION OF FINANCIAL & COMPLIANCE RISK:</strong> By utilizing our calculators or generating invoices, you explicitly acknowledge that any final decisions, tax returns, payroll calculations, sales invoice pricing, inventory budgeting, or corporate fundraising strategies are executed at your own sole risk. SimplyTools, its founders, and operators shall not be held liable, under any scenario, for tax shortfalls, business losses, missed runway projections, audit failures, interest levies, payment collection failures, or compliance penalties.
              </p>
              <p>
                <strong>1.6 No Liability for System Downtime or Logic Bugs:</strong> SimplyTools shall have no liability for system errors, decimal rounding mismatches, browser cache issues, server latency, or temporary service interruptions experienced during planning runs.
              </p>
            </div>
          )}
